...
with st.sidebar:
    ...
    sel_carriers = st.multiselect(
        "Carrier", sorted(CARRIERS), default=sorted(CARRIERS)
    )
    sel_facilities = st.multiselect(
        "Facility", sorted(FACILITIES), default=sorted(FACILITIES)
    )
...
def render_detail(row: pd.Series):
    ...
    with rc2:
        with st.container(border=True):
            ...
            # Heuristic factor weights based on shipment values
    ...
    with st.container(border=True):
        ...
        st.caption(
            f"Last 10 shipments via {row['carrier']} to a similar facility"
        )
        similar = (
...
if st.session_state["selected_shipment"] is not None:
    ...
    # Handle row selection → open detail view
